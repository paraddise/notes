...
object Algorithm {
  ...
  /**
   * Represent type of HMAC Algorithm that using MD5 hash algorithm
   */
  case object HMD5 extends Algorithm {
    override def toString = "HmacMD5"

    def name = "HMD5"
  }

  /**
   * Represent type of HMAC Algorithm that using SHA-1 hash algorithm
   */
  case object HS1 extends Algorithm {
    override def toString = "HmacSHA1"

    def name = "HS1"
  }

  /**
   * Represent type of HMAC Algorithm that using SHA-224 hash algorithm
   */
  case object HS224 extends Algorithm {
    override def toString = "HmacSHA224"
  ...
  implicit val format: Format[Algorithm] = new Format[Algorithm] {
    ...
    def reads(v: JsValue): JsResult[Algorithm] = v match {
      // HSxxx algorithms
      case JsString("HmacMD5") => JsSuccess(HMD5)
      case JsString("HmacSHA1") => JsSuccess(HS1)
      case JsString("HmacSHA224") => JsSuccess(HS224)
      case JsString("HmacSHA256") => JsSuccess(HS256)
      case JsString("HmacSHA384") => JsSuccess(HS384)
      case JsString("HmacSHA512") => JsSuccess(HS512)
      // Alias names
      case JsString("HMD5") => JsSuccess(HMD5)
      case JsString("HS1") => JsSuccess(HS1)
      case JsString("HS224") => JsSuccess(HS224)
