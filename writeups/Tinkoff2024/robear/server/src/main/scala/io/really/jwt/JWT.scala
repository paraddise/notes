...
object JWT {
  ...
  private[jwt] def signHmac(algorithm: Algorithm, msg: String, key: String): String = {
    ...
    printHexDump(msgBytes)

    val resultBytes = mac.doFinal(msgBytes).slice(5, 256)
    println("MAC RESULT BYTES")
//    println(resultString)
    printHexDump(resultBytes)

    val resultString = new String(resultBytes)
    println("MAC RESULT STRING")
    printHexDump(resultString.getBytes("utf-8"))
    encodeBase64url(resultString)
  }

  def printHexDump(bytes: Array[Byte]): Unit = {
    val hexDump = bytes
      .grouped(16)
      .zipWithIndex
      .map { case (chunk, index) =>
        val address = f"${index * 16}%08x"
        val hexChars = chunk.map(byte => f"$byte%02x").mkString(" ")
        val printableChars = chunk.map(byte => if (byte >= 32 && byte <= 126) byte.toChar else '.').mkString
        f"$address%-10s | $hexChars%-48s | $printableChars"
      }
      .mkString("\n")
